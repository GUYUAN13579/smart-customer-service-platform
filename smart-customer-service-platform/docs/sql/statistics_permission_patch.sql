...
FROM (
    SELECT 'statistics:agent-workload' AS permission_code, '查看客服负载统计' AS permission_name,
           '/api/v1/statistics/agent-workloads' AS resource_path, 701 AS sort_order
    UNION ALL
...
JOIN sys_permission p ON p.permission_code IN (
    'statistics:agent-workload', 'statistics:ticket-trend', 'statistics:sla-performance'
)
